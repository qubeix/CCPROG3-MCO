 ...
 * series entry with:<br>
 * the inherited methods of: assigning a status (and modifying it), assigning a
 * rating, and assigning a review; and<br>
 * the unique methods of: assigning the number of episodes per season, updating
 * the series entry's current episode and season progress, and returning its
 * information via String
 */
public class Series extends MediaEntry {
    ...
    public void updateProgress() {
        if (currentEpisode < episodeCount[currentSeason - 1]) // If the current episode has not reached the last episode
        {
            ...
            if (currentEpisode == episodeCount[SEASONCOUNT - 1] && currentSeason == SEASONCOUNT) // If it is the last
    ...
    /**
     * Returns the progress of the series, in the format of:
     * "{@literal Season <current season>: <current episode>/<total episodes in current season> Episodes}"
     * 
     * @return the progress
     */
    public String getProgress() {
        return "Season " + currentSeason + ": " + currentEpisode + "/" + episodeCount[currentSeason - 1]
